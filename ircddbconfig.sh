...
fix_dstar_gw_init_script_debian() {
  ...
  then
    sed '
      ...
      N
      N
      s/^\(start.\n    start\n\)\(    ;;\)$/\1    \/usr\/sbin\/invoke-rc.d ircddbd start\n\2/
      }
      /^stop)$/ {
      N
      N
      s/^\(stop.\n\)\(    stop\n    ;;\)$/\1    \/usr\/sbin\/invoke-rc.d ircddbd stop\n\2/
      }
      /^restart)$/ {
      N
      N
      N
      s/^\(restart.\n\)\(    stop\n    start\n\)\(    ;;\)$/\1    \/usr\/sbin\/invoke-rc.d ircddbd stop\n\2    \/usr\/sbin\/invoke-rc.d ircddbd start\n\3/
      }
     '  < "$DBAK"  > "$D"
  fi
}
...
then
  ...
  then
    ...
    then
      ...
      $RCCMD ircddbd stop
      $RCCMD ircddbmhd stop
      $RCCMD ircddbd start
      $RCCMD ircddbmhd start
    fi
